...
## Update 03 Agustus 2026
- **Backend & CI/CD**:
  - Memperbaiki *error* TypeScript di `splitController.ts` akibat `sendGroupMessageViaGowa` di `BlastController.ts` belum di-`export`.
  - Memperbarui workflow CI/CD (`kainest-be-cicd.yml`) dan Ansible playbook (`deploy.yml`) untuk menginjeksi variabel lingkungan `RECEIPT_AI_TOKEN` dan `RECEIPT_AI_URL` ke `.env` saat deployment.
- **Receipt AI Service**:
  - Menambahkan URL DNS Backend Kainest ke daftar *allowed CORS origins* pada `app/main.py`.
- **Frontend & Deployment**:
  - Memperbarui halaman fitur Split Bill (`SplitBillPage.vue`, `SharedSplitPage.vue`).
  - Mempublikasikan seluruh pembaruan pada Kainest Backend (`staging`), Kainest Frontend (`staging`), Receipt AI Service (`main`), dan WhatsApp Bot (`staging`).
