...
function isTodayResetDay(payday: number): boolean {
  ...
  if (payday > lastDay && today === lastDay) return true;

  return false;
}

// ── Core: Kalkulasi ringkasan keuangan siklus yang berakhir ─────────────────
async function calculateCycleSummary(userId: string): Promise<{
  salary: number;
  totalExpense: number;
  totalIncome: number;
  surplus: number;
...
async function runMonthlyReset(): Promise<void> {
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (!isTodayResetDay(user.payday)) continue;
