 ...
 */
export const classifyTransactionUseCase = async (
  userId: string,
  text: string
  ...
  try {
    ...
    const fallbackCategory = allCategories.find(
      (c) => c.name === "Lain-lain" && c.isDefault
    );
    ...
    if (userPockets.length > 0) {
      ...
      categoryListText = allCategories
        .map(
          (c) =>
            `- ID: ${c.id} | Nama: ${c.name} | Keywords: ${(c.keywords || []).join(", ")}`
        )
        .join("\n");
    }
...
`;
    ...
    const chosenCategory =
      userPockets.find((p) => p.category.id === chosenId)?.category ||
