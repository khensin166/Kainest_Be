 ...
 */
export const calculateAndSaveSplitController = async (c: Context) => {
  try {
    ...
    if (!response.ok) {
      ...
      return c.json({ error: "Gagal menghitung di AI Service", details: errorData }, response.status as any);
    }
 ...
 */
export const scanReceiptController = async (c: Context) => {
  try {
    const session = await auth.api.getSession({ headers: c.req.raw.headers });
    if (!session || !session.user) {
      return c.json({ error: "Unauthorized" }, 401);
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return c.json({ error: "Gagal scan struk di AI Service", details: errorData }, response.status as any);
    }
 ...
 */
export const getSharedSplitController = async (c: Context) => {
  try {
 ...
 */
export const blastSplitBillController = async (c: Context) => {
  try {
    const session = await auth.api.getSession({ headers: c.req.raw.headers });
    if (!session || !session.user) {
      return c.json({ error: "Unauthorized" }, 401);
