...
// === CONTROLLERS ===
export const getTodosController = async (c) => {
    // 1. Ambil ID
...
export const createTodoController = async (c) => {
    const userId = c.get("userId");
    const coupleId = await getCoupleId(c);
    // 2. Controller mengambil keputusan
    if (!coupleId) {
...
export const updateTodoController = async (c) => {
    const coupleId = await getCoupleId(c);
    // 2. Controller mengambil keputusan
    if (!coupleId) {
        c.status(403);
...
export const deleteTodoController = async (c) => {
    const coupleId = await getCoupleId(c);
    // 2. Controller mengambil keputusan
    if (!coupleId) {
        c.status(403);
