...
export const setupMonthlyBudgetUseCase = async (data) => {
    try {
        ...
        const findCatId = (name) => categories.find((c) => c.name.toLowerCase().includes(name))?.id;
        ...
        const promises = [];
        if (idMakan)
            promises.push(budgetRepository.upsertBudget(userId, idMakan, period, foodBudget));
        if (idTrans)
            promises.push(budgetRepository.upsertBudget(userId, idTrans, period, transportBudget));
        if (idKos)
            promises.push(budgetRepository.upsertBudget(userId, idKos, period, rentBudget));
        if (idHiburan)
            promises.push(budgetRepository.upsertBudget(userId, idHiburan, period, entertainmentBudget));
