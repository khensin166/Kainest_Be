...
export const saveConfigController = async (c) => {
    ...
    const result = await saveConfigUseCase(userId, body.baseUrl, body.adminSecret);
    if (!result.success) {
        c.status(result.status);
    }
    return c.json(result);
};
export const getConfigController = async (c) => {
